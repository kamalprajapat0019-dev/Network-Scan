...
while ($keepScanning) {
    Write-Color "`n=======================================================================" "Cyan"
    Write-Color "                 RUNNING NETWORK SCAN ($scanCount/$maxScans)               " "Cyan"
    Write-Color "=======================================================================" "Cyan"
    ...
    try {
        ...
        if ($response.success) {
            ...
            if ($response.error -match "limit exceeded|Only 5 scans") {
                $limitExceeded = $true
            }
        ...
        Write-Color "Error Details: $errorMsg" "Red"
        
        if ($errorMsg -match "limit exceeded|Only 5 scans|400") {
            # Let's check if the raw response was a bad request with the limit message
            try {
                if ($_.Exception.Response) {
                    $streamReader = New-Object System.IO.StreamReader($_.Exception.Response.GetResponseStream())
                    $respBody = $streamReader.ReadToEnd()
                    $respObj = ConvertFrom-Json $respBody
                    if ($respObj.error -match "limit exceeded|Only 5 scans") {
                        Write-Color "Server message: $($respObj.error)" "Red"
                        $limitExceeded = $true
                    }
                }
            } catch {}
    ...
    if ($limitExceeded) {
        Write-Color "`n[!] Scan aborted: The maximum allowed limit of 5 scans has been reached on the server!" "Red"
        $keepScanning = $false
        break
    }
    
    if ($scanCount -ge $maxScans) {
        Write-Color "`n[!] Maximum session scan limit reached ($maxScans/$maxScans). Exiting scanner." "Yellow"
        $keepScanning = $false
    } else {
        Write-Color "`n"
        Write-Color "=======================================================================" "Cyan"
        Write-Color "                         RESCAN OPTION MENU                            " "Cyan"
        Write-Color "=======================================================================" "Cyan"
        Write-Color "  [R] Rescan IPs now (Remaining: $($maxScans - $scanCount))" "Green"
        Write-Color "  [E] Exit Scanner" "Gray"
        Write-Color "=======================================================================" "Cyan"
        Write-Color "Select option [R/E] (Default: E): " -NoNewLine "Yellow"
